...
public class CTLabDAO extends BaseJDBCDAO {
	...
	private void saveParticipant(Connection con, Participant participant)
			throws SQLException {
		...
		if (rc && gc) {
			...
			ps = con
					.prepareStatement("insert into PARTICIPANT (PERSON_ID, BIRTH_DATE, BIRTH_DATE_ORIG, INITIALS, LAST_NAME, FIRST_NAME, ADM_GNDR_CONCEPT_DESCRIPTOR_ID,RACE_CONCEPT_DESCRIPTOR_ID )  values(?,?,?,?,?,?,?,?)");
			ps.setLong(7, genderCDId);
			ps.setLong(8, raceCDId);
		} else if (!rc && !gc) {

			ps = con
					.prepareStatement("insert into PARTICIPANT (PERSON_ID, BIRTH_DATE, BIRTH_DATE_ORIG, INITIALS, LAST_NAME, FIRST_NAME, MARITAL_STATUS_CODE, ZIP_CODE, COUNTRY_CODE )  values(?,?,?,?,?,?,?,?,?)");

		} else if (!rc) {
			ps = con
					.prepareStatement("insert into PARTICIPANT (PERSON_ID, BIRTH_DATE, BIRTH_DATE_ORIG, INITIALS, LAST_NAME, FIRST_NAME, MARITAL_STATUS_CODE, ZIP_CODE, COUNTRY_CODE, ADM_GNDR_CONCEPT_DESCRIPTOR_ID )  values(?,?,?,?,?,?,?,?,?,?)");
			ps.setLong(7, genderCDId);

		} else if (!gc) {
			ps = con
					.prepareStatement("insert into PARTICIPANT (PERSON_ID, BIRTH_DATE, BIRTH_DATE_ORIG, INITIALS, LAST_NAME, FIRST_NAME, MARITAL_STATUS_CODE, ZIP_CODE, COUNTRY_CODE, RACE_CONCEPT_DESCRIPTOR_ID )  values(?,?,?,?,?,?,?,?,?,?)");
