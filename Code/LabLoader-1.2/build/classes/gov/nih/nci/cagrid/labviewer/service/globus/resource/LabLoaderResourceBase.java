...
import gov.nih.nci.cagrid.labviewer.service.LabLoaderConfiguration;
import java.io.File;
 ...
 * 
 */
public abstract class LabLoaderResourceBase extends ReflectionResource implements Resource
                                                  ,SecureResource
                                                  {

	static final Log logger = LogFactory.getLog(LabLoaderResourceBase.class);

	private LabLoaderResourceConfiguration configuration;
	...
	public gov.nih.nci.cagrid.metadata.ServiceMetadata getServiceMetadata(){
		return ((LabLoaderResourceProperties) getResourceBean()).getServiceMetadata();
	}
	
	public void setServiceMetadata(gov.nih.nci.cagrid.metadata.ServiceMetadata serviceMetadata ) throws ResourceException {
        ResourceProperty prop = getResourcePropertySet().get(LabLoaderConstants.SERVICEMETADATA);
	...
	private void loadServiceMetadataFromFile() {
      if(getServiceMetadata()==null){
		try {
			...
			((LabLoaderResourceProperties) this.getResourceBean()).setServiceMetadata((gov.nih.nci.cagrid.metadata.ServiceMetadata) Utils.deserializeDocument(dataFile.getAbsolutePath(),
